...
public interface AttributeExamples {
    ...
    static List<NamedAttribute> attributesByExampleName(String exampleName, String userName) {
        switch (exampleName) {
            case CRASH_OKTA:
                return attributesThatCrashOkta(userName);
            case WORKS_FOR_OKTA:
                return attributesThatWorkForOkta(userName);
            default:
                throw new IllegalArgumentException("Unknown example name '" + exampleName + "'");
        }
    }
